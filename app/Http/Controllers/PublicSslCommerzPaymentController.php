        ...
        if($order_detials->order_status=='Pending')
        {
            ...
            if($validation == TRUE) 
            {
                ...
                */ 
                ...
                $donation=Donation::findOrFail(Session::get('donation_id'));
                $total=Session::get('transaction_data')['grand_total'];
                $newDonation=$donation->total_collection+$total;
                $updateDonation=$donation->total_collection+$total;

                if($newDonation<=$donation->donation_needed){
                    $update_donation = DB::table('donations')
                            ->where('id', Session::get('donation_id'))
                            ->update(['total_collection' =>$updateDonation ]);
                   ...
                   $update_donation = DB::table('donations')
                            ->where('id', Session::get('donation_id'))
                            ->update(['total_collection' =>$donation->donation_needed ]); 
                    $update_profit=DB::table('orders')
                            ->where('order_id', Session::get('order_id'))
                            ->update(['profit' =>($newDonation-$donation->donation_needed) ]); 
                    return redirect('/transaction/success')->with('status','you have contributed!');
                }
